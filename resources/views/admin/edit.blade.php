...
@section('content')
<form  action="/about/{{$edit->id}}/update"  method="POST" enctype="multipart/form-data">
	@csrf
	@method("PUT")
	<div  class="mb-3">
	...
	<div  class="mb-3">
		...
		<input  type="number"  value={{$edit->title2}}  name="title2"  class="form-control"  id="quantity"  aria-describedby="emailHelp">
	</div>
	<div  class="mb-3">
		<label  for="image"  class="form-label">text1</label>
		<textarea  type="text" name="text1"  class="form-control"  id="image"  aria-describedby="emailHelp"  placeholder="insérez le nom de votre fichier suivi de son extension" >{{ $edit->text1 }}</textarea>
	</div>
	<div  class="mb-3">
